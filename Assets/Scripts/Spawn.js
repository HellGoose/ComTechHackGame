...
function Start () {
	fonts = Resources.LoadAll("Fonts",Font);
...
function DropBall() {
	...
	var word = Instantiate(target,transform.position+offset, transform.rotation);
	var font = fonts[Random.Range(0,fonts.length)];
	word.GetComponent(TextMesh).font = font;
	word.GetComponent(MeshRenderer).materials[0]=font.material;
	word.GetComponent(MeshRenderer).sharedMaterial=font.material;
	var difficulty = gameObject.Find("Score").GetComponent(Score).GetScore();
	var wordlist;
	wordlist = words[Random.Range(0,difficulty*words.length-1)];
	var text = wordlist[Random.Range(0,wordlist.length-1)];
	word.GetComponent(TextMesh).text = text;
//	Options.say(text);
	
	word.gameObject.SetActiveRecursively(false);
	word.gameObject.active=true;
